        ...
        <div className="min-h-screen bg-gradient-to-b from-river-50 via-emerald-50 to-white pb-12">
            <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-slate-200">
                <div className="max-w-md mx-auto px-3 py-3 flex items-center gap-2">
                    <button onClick={() => nav("/map")} className="p-2 -ml-1 rounded-full hover:bg-slate-100" data-testid="friends-back">
                ...
                <div className="max-w-md mx-auto px-2 pb-2 flex gap-1">
                    {TABS.map(({ id, label, Icon }) => (
                        <button
                            key={id}
                            onClick={() => setTab(id)}
                            className={`flex-1 px-3 py-2 rounded-xl flex items-center justify-center gap-1 text-xs font-bold transition ${tab === id ? "bg-river-500 text-white shadow-md" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
            ...
            {friends.map((f) => (
                <li key={f.camper_id} className="bg-white rounded-2xl border border-slate-200 p-3 flex items-center gap-3" data-testid={`friend-row-${f.camper_id}`}>
                    ...
                    <Button
                        ...
                        className="bg-amber-400 hover:bg-amber-500 text-amber-950 rounded-full text-xs font-black px-3 py-2 disabled:opacity-50"
